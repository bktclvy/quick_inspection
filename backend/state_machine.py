...
class InspectionStateMachine:
    def __init__(self):
        ...
        # 取出し検知パラメータ（背景テンプレートマッチ）
        self.removal_bg_threshold: float = 0.85  # 背景マッチスコアがこれ以上→取出し確認
    ...
    def process_frame_unified(self,
        ...
        """1フレームを処理する。設置検知（ROIテンプレート）＋取出検知（背景テンプレート）の統一モード。

        match_scores: 各ROIの拡大領域マッチスコア {roi_id: 0-1 or None}
        bg_match:     背景テンプレートマッチスコア (0-1)。高い＝背景に近い＝製品なし。Noneなら背景未登録。
    ...
    def _process_frame_unified_internal(self,
        ...
        # ── WAITING_REMOVAL: 取出し確認（背景テンプレートマッチ） ──
        if self.state == InspectionState.WAITING_REMOVAL:
            # 背景テンプレートのマッチスコアが高い → 背景に戻った → 取出し確認
